...
const dominantDirection = str => {
    
// Функция определения кода каждого отдельного символа utf-16 для переданной строки
    
    const loopDetect = str => {
        let resultArr = [];
        for (let sumbol of str) {
            if (characterScript(sumbol.codePointAt())) {
                resultArr.push(characterScript(sumbol.codePointAt()));
            } 
        }
        return resultArr;
    }

// Функция опередления к какому языку относится переданный utf-16 код. 
// Выдается объект содержащий все свойства языка
...
// Основная функция проверяет массив содержащий набор языков и считает количество повторов для каждого 
// из направлений текста, затем сравнивает и находит наибольшее значение среди 3 направлений

    let arr = loopDetect(str);
        ltr = 0,
        rtl = 0,
        ttb = 0;
    
    for (obj of arr) {
        if (obj.direction == 'ltr') {
            ltr++;
        }
        else if (obj.direction == 'rtl') {
            rtl++;
        }
        else if (obj.direction == 'ttb') {
            ttb++;
        }
    }
    if (ltr > rtl && ltr > ttb) {
